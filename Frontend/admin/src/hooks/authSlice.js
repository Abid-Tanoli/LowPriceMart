...
const initialState = {
  ...
  userInfo: getSafeJSON("userInfo") || null,
  loading: false,
  error: null,
};
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(initializeUser.fulfilled, (state, action) => { state.loading = false; state.userInfo = action.payload; state.error = null; })
      .addCase(initializeUser.rejected, (state, action) => { state.loading = false; state.token = null; state.userInfo = null; state.error = action.payload; })

      // Register
      .addCase(registerUserThunk.pending, (state) => { state.loading = true; state.error = null; })
      .addCase(registerUserThunk.fulfilled, (state, action) => { state.loading = false; state.token = action.payload.token; state.userInfo = action.payload.user; state.error = null; })
      ...
      .addCase(loginUserThunk.fulfilled, (state, action) => { state.loading = false; state.token = action.payload.token; state.userInfo = action.payload.user; state.error = null; })
